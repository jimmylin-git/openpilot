...
def _cores_for_core_mode(core_mode: int) -> list[int]:
    ...
    allowed = set(INITIAL_ALLOWED_CORES)
    return [core for core in DEFAULT_REALTIME_CORES if core in allowed] or _all_realtime_cores()
...
def _parse_realtime_cores(text: str) -> list[int]:
    ...
    for value in text.split(","):
        ...
        if core in allowed:
            cores.append(core)
    return sorted(set(cores)) or _all_realtime_cores()
...
def _is_usb_disconnect_error(exc: BaseException) -> bool:
    ...
    while current is not None and id(current) not in seen:
        seen.add(id(current))
        if any(marker in str(current).lower() for marker in USB_DISCONNECT_TEXT):
            return True
...
def _run_cluster_once(
    ...
    from selfdrive.dp import cluster_run
    ...
    while True:
        ...
        try:
            ...
            if (
                ...
                or next_orientation != orientation
            ...
            if _is_usb_disconnect_error(exc):
                print(
                    f"[cluster_autorun] USB display disconnected; retrying in "
                    f"{RETRY_INTERVAL_S:.0f}s",
                    flush=True,
                )
            else:
                print(
                    f"[cluster_autorun] cluster HUD failed: {exc}; "
                    f"retrying in {RETRY_INTERVAL_S:.0f}s",
                    flush=True,
                )
                traceback.print_exc()
