...
try:
    ...
    CATALOG = ()
    MAP_HZ_MAX = 60
    CarrotNaviReceiver = Any
    ItemRecord = Any
    create_app = None
    build_carrot_navi_payload = None


def _require_carrot_navi() -> None:
    if web is None or create_app is None or build_carrot_navi_payload is None:
        raise RuntimeError(
            "aiohttp and Carrot Navi modules are required for standalone navigation; "
            "install openpilot/selfdrive/carrot/carrot_navi.py and "
            "carrot_navi_cereal.py"
        )
