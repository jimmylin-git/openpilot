...
def _load_library(*names: str) -> ctypes.CDLL:
  ...
  raise DirectNv12DmabufError(f"failed to load {names[0]}: {'; '.join(errors)}")
...
class GlesDmabufRenderTargetPool:
  ...
  def __init__(self) -> None:
    self._egl = _load_library("libEGL.so.1", "libEGL.so")
    self._gles = _load_library("libGLESv2.so.2", "libGLESv2.so")
...
def create_tici_nv12_dmabuf_pool() -> GlesDmabufRenderTargetPool | None:
  if os.environ.get("CLUSTER_NV12_DMABUF_OUTPUT", "1") == "0" or not Path("/TICI").is_file():
    return None
  # Only the Adreno (comma backend) EGL can import the Venus encoder buffers.
  if os.environ.get("RAYLIB_BACKEND", "").strip().lower() in ("headless", "desktop"):
    return None
