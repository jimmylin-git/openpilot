  ...
  def _copyin(self, dest:HCQBuffer, src:memoryview):
    ...
    for c in range(nchunks):
      ...
      with usbgpu_bus_lock():
        usb.usb.control_write(0xF2, value=wire // 512, index=(seq & 1) * 16 | (ceildiv(wire, 0x4000) << 8))
        rd_mv = usb.usb.control_read(0xE4, 8, value=FENCE)
      if int.from_bytes(rd_mv, 'little') < seq - 1: wait_drain(seq - 1)  # rare: the drain lagged; spin on fresh reads
      with usbgpu_bus_lock():
        tag = usb.usb.bulk_write_async(buf[:wire])
        usb.usb.bulk_wait(tag)
